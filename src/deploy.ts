/**
 * Deploy the WanningGate counter contract to a Midnight network (undeployed by default; use --network preview|preprod for public networks).
 *
...
import { createCounterPrivateState } from './witnesses';
import { CLAIM_SECRET_FILE, getOrCreateClaimSecret } from './claim-secret';
import { PRIVATE_STATE_ID, loadCompiledContract, zkConfigPath } from './contract';
...
// ─── Private state ─────────────────────────────────────────────────────────────
//
// The counter contract reads its secret through the `memberSecret()` witness, so
// private state must be supplied at deploy time. The secret never leaves this
// machine — only the commitment tag it derives reaches the ledger.

const CLAIM = getOrCreateClaimSecret();
const PRIVATE_STATE = createCounterPrivateState(CLAIM.secret);
if (CLAIM.created) {
  console.log(
    `\n  New claim secret generated and saved to ${CLAIM_SECRET_FILE} (gitignored).`,
  );
  console.log('  This is private witness input — it is never sent to the network.\n');
}
...
async function createProviders(walletCtx: WalletContext) {
  ...
  return {
    privateStateProvider: levelPrivateStateProvider({
      privateStateStoreName: 'wanning-gate-state',
      accountId,
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log(`║  Deploy WanningGate counter to ${network}`);
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      // initialPrivateState. The counter contract's `memberSecret()` witness
      // reads `claimSecret` from here, so it must be present or proving the
      // first claim() fails. args is the contract constructor's arguments:
      // empty for this contract's no-arg constructor. (Statically-typed
